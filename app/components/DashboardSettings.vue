<template>
  <UCard v-if="store.config && localBackgroundConfig" variant="glassDark" class="border-0! shadow-none! overflow-hidden"
    :ui="{
      body: 'p-8',
    }">
    <template #header>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
      ...
      <div class="space-y-8">
        ...
        <UFormField :label="$t('manage.preferences.transitionMode')"
          ...
          <USelect v-model="localBackgroundConfig.transitionMode" :items="transitionModeOptions" size="xl"
            class="w-full" />
        </UFormField>

        <UFormField :label="$t('manage.preferences.playbackOrder')"
          :description="$t('manage.preferences.playbackDescription')">
          <USelect v-model="localBackgroundConfig.playbackOrder" :items="[...PLAYBACK_MODES]" size="xl"
            class="w-full" />
        </UFormField>

        <UFormField :label="$t('manage.preferences.interval')"
          :description="$t('manage.preferences.intervalDescription')">
          <UInput v-model.number="localBackgroundConfig.interval" type="number" step="1000" min="1000" size="xl"
            class="w-full" />
        </UFormField>

        <UFormField :label="$t('manage.preferences.videoPlayback')"
          :description="$t('manage.preferences.videoDescription')">
          <USelect v-model="localBackgroundConfig.videoPlaybackMode" :items="[...VIDEO_PLAYBACK_MODES]" size="xl"
            class="w-full" />
        </UFormField>

        <UFormField :label="$t('manage.preferences.lowPowerMode')"
