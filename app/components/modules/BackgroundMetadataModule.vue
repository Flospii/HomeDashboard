<template>
  <BaseModule v-if="shouldShow">
    <div class="flex flex-col text-white max-w-[300px] space-y-2">
      ...
      <div v-if="hasSecondaryInfo && metadata" :class="[
        'space-y-1.5',
        hasPrimaryInfo ? 'pt-2 border-t border-white/10' : '',
      ]">
        <h3 v-if="showFileName" class="text-xs md:text-sm font-medium text-white/40 break-all leading-tight italic">
          {{ metadata.fileName }}
        </h3>
        ...
        <div
          ...
          <div v-if="showModifiedAt && metadata.modifiedAt" class="flex items-center space-x-1">
            <UIcon name="i-heroicons-pencil-square" class="w-2.5 h-2.5" />
            <span>{{ $t("manage.modules.metadata.modifiedAt") }}:
...
const metadata = computed<MediaMetadata | undefined>(
  () => store.currentBackground?.metadata
);
