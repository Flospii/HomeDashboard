<template>
  <BaseModule v-if="hasVisibleMetadata">
    <div class="flex flex-col space-y-3 text-white">
      <div v-if="showFileName && metadata?.fileName" class="flex items-center space-x-2">
      ...
      <div v-if="showGPS && metadata?.gps" class="flex items-center space-x-2 text-sm text-white font-bold">
        <UIcon name="i-heroicons-map-pin" class="w-4 h-4" />
        <span>{{ metadata.gps.latitude.toFixed(4) }},
      ...
      " class="pt-1 space-y-1">
        <div v-if="showCreatedAt && metadata?.createdAt"
          class="flex items-center space-x-2 text-sm opacity-40 font-bold uppercase tracking-tighter">
          <UIcon name="i-heroicons-calendar" class="w-4 h-4" />
          <span>{{ $t("modules.background-metadata.createdAt") }}:
            {{ formatDate(metadata.createdAt) }}</span>
        </div>
        <div v-if="showModifiedAt && metadata?.modifiedAt"
          class="flex items-center space-x-2 text-sm opacity-40 font-bold uppercase tracking-tighter">
          <UIcon name="i-heroicons-pencil-square" class="w-4 h-4" />
          <span>{{ $t("modules.background-metadata.modifiedAt") }}:
      ...
      " class="pt-1 space-y-1 text-sm font-bold">
        <div v-if="showCamera && metadata?.camera" class="flex items-center space-x-2 opacity-80">
          <UIcon name="i-heroicons-camera" class="w-4 h-4" />
          <span>{{ metadata.camera }}</span>
        </div>

        <div class="flex flex-wrap gap-x-3 gap-y-1 text-xs opacity-60 pt-0.5"
          v-if="(showFocalLength && metadata?.focalLength) || (showAperture && metadata?.aperture) || (showExposureTime && metadata?.exposureTime) || (showISO && metadata?.iso)">
          <div v-if="showFocalLength && metadata?.focalLength">{{ metadata.focalLength }}mm</div>
          <div v-if="showAperture && metadata?.aperture">{{ metadata.aperture }}</div>
          <div v-if="showExposureTime && metadata?.exposureTime">{{ metadata.exposureTime }}s</div>
...
const hasVisibleMetadata = computed(() => {
  ...
  const hasExif = (props.showFocalLength && m.focalLength) || (props.showISO && m.iso) || (props.showAperture && m.aperture) || (props.showExposureTime && m.exposureTime);
