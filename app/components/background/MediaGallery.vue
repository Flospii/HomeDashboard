  ...
  <div class="space-y-8">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-6 border-b border-default/30">
        ...
        <div class="flex items-center space-x-4">
            ...
            <div class="hidden sm:flex items-center space-x-2 text-[10px] text-default/40 uppercase tracking-widest font-black">
                <UIcon name="i-lucide-database" class="w-3 h-3" />
                <span>{{ media.length }} {{ $t("manage.backgrounds.items", { count: media.length }).replace(media.length.toString(), "").trim() }}</span>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2">
            <UButton
                v-if="!isSharedView && currentFolder"
    ...
    <div v-else class="space-y-12 animate-in fade-in duration-500">
      <div v-if="folders.length > 0 || !isSharedView" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
        ...
        >
          <div class="w-12 h-12 bg-primary-500/10 rounded-full flex items-center justify-center transition-transform hover:scale-110">
        ...
        >
          <div class="w-12 h-12 bg-default/5 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform">
          ...
          <!-- Folder Actions -->
          <div v-if="!isSharedView" class="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <UButton icon="i-lucide-pencil" color="neutral" variant="soft" size="xs" @click.stop="openRenameModal(folder)" />
            <UButton icon="i-lucide-trash-2" color="error" variant="soft" size="xs" @click.stop="emit('deleteFolder', folder)" />
          </div>
        </div>
      </div>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <div v-for="item in media" :key="item.url" class="relative group aspect-video overflow-hidden border border-default bg-default/40 shadow-xl">
          <!-- Preview -->
          <img v-if="item.type === 'image'" :src="item.url" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
          <video v-else :src="item.url" class="w-full h-full object-cover" muted />

          <!-- Overlay Actions -->
          <div class="absolute inset-0 sm:bg-default/60 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-4">
            <UButton
              v-if="!isSharedView"
              icon="i-lucide-clock"
              color="primary"
          ...
          <!-- Type Badge -->
          <div class="absolute top-4 left-4 px-3 py-1 bg-default/60 text-[10px] font-bold uppercase tracking-widest text-default/80 border border-default">
    ...
    <UModal v-model:open="showRenameModal" :title="$t('manage.backgrounds.renameFolder')">
      <template #body>
        <UFormField :label="$t('manage.backgrounds.newName')">
          <UInput v-model="renameValue" :placeholder="folderToRename" class="w-full" autofocus @keyup.enter="handleRename" />
        </UFormField>
      </template>
    ...
    <UModal v-model:open="showCreateFolderModal" :title="$t('manage.backgrounds.newFolder')">
      <template #body>
        <UFormField :label="$t('manage.backgrounds.folderName')">
          <UInput v-model="newFolderNameInput" placeholder="e.g. My Trip" class="w-full" autofocus @keyup.enter="handleCreate" />
        </UFormField>
      </template>
      <template #footer="{ close }">
        <div class="flex justify-end gap-3 w-full">
