<template>
  <div class="space-y-8">
    <div
      class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-6 border-b border-default/30"
    >
    ...
    <div v-else class="space-y-12 animate-in fade-in duration-500">
      <div
        v-if="folders.length > 0 || !isSharedView"
      ...
      <div
        v-if="media.length === 0"
        class="text-center py-20 bg-default/5 border border-default rounded-xl"
      >
        <UIcon
          name="i-lucide-image"
      ...
      >
        ...
        >
          ...
          >
            <UIcon
              :name="
                item.type === 'image' ? 'i-lucide-image' : 'i-lucide-video'
              "
              class="w-3 h-3"
            />
