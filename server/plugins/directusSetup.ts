...
import { createDirectus, rest, login, readCollections, createCollection, readPermissions, createPermission, deletePermission, readPolicies, createPolicy, readItems, createItem, readItem, staticToken } from '@directus/sdk';
...
export default defineNitroPlugin(async (nitroApp) => {
  ...
  try {
    console.log("[Server] DirectusSetup | Checking if 'dashboard_config' collection exists...");
    ...
    let authResult;
    try {
      authResult = await client.request(login({ email: "admin@example.com", password: "admin" }));
    } catch (e) {
      console.log("[Server] DirectusSetup | Could not authenticate using default admin account. Assuming custom setup.");
      return;
    }
    ...
    try {
      ...
      if (publicPolicy) {
        ...
        publicPolicyId = access?.data?.[0]?.policy || access?.[0]?.policy || null;
      }
      
      if (!publicPolicyId) {
        console.log("[Server] DirectusSetup | Creating Public Policy...");
        const newPolicy = await adminClient.request(createPolicy({ 
             name: "Public", 
             icon: "public", 
    ...
    const revokePublicAccess = async (collection: string, action: string) => {
      ...
      try {
        const perms = await adminClient.request(readPermissions({
          filter: {
    ...
    const grantPublicAccess = async (collection: string, action: string = "read") => {
      if (!publicPolicyId) return;
      try {
        const existing = await adminClient.request(readPermissions({
          filter: {
            collection: { _eq: collection },
            policy: { _eq: publicPolicyId },
            action: { _eq: action }
    ...
    try {
      ...
      if (cols && Array.isArray(cols)) {
        collectionExists = cols.some((c: any) => c.collection === 'dashboard_config');
      }
    } catch (e) {}

    if (!collectionExists) {
      console.log("[Server] DirectusSetup | Collection not found. Creating it automatically...");

      await adminClient.request(createCollection({
      ...
      try {
        await adminClient.request(createItem('dashboard_config', {
          id: "1",
        ...
        console.error("[Server] DirectusSetup | Failed to seed default data, retrying without explicit ID...");
        await adminClient.request(createItem('dashboard_config', {
          background: initialConfig.background,
          language: initialConfig.language,
          modules: initialConfig.modules
      ...
      console.log("[Server] DirectusSetup | 'dashboard_config' already exists. Synchronizing permissions...");
      ...
      try {
        ...
        await adminClient.request(createItem('dashboard_config', {
          ...
          return adminClient.request(createItem('dashboard_config', {
            background: initialConfig.background,
            language: initialConfig.language,
            modules: initialConfig.modules
          }));
